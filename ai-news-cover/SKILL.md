...
`coverHook` 规则——它是封面上那行**超大主标题**，决定缩略图的点击率：

- **10–16 个汉字**，宁短勿长（字越少越大越醒目；超过 ~18 字会在画面里被迫缩小换行）
- 取当天**最重要那条**新闻（通常 `overview[0]` / 第一个 keypoint）的爆点
- 含**主体 + 强动作/关键数字**，像标题党但**不许虚假或夸大**，不与原文矛盾
- 不要 emoji、不要感叹号堆砌、不要书名号
- ✅ "美政府强令下架 Fable 5"
- ✅ "SpaceX 750 亿 IPO 押注 AI"
- ❌ "今日 AI 大事件"（太空泛）
- ❌ "震惊！AI 圈彻底炸锅了"（虚张）
...
- 选**产品名 / 关键数字 / 强动词**之一，通常是观众一眼要捕捉的那个词
- 只选**一个**，别贪多——多个高亮会稀释焦点
- 例：`coverHook: "美政府强令下架 Fable 5"` → `coverHookHighlight: "Fable 5"`；`coverHook: "SpaceX 750 亿 IPO 押注 AI"` → `coverHookHighlight: "750 亿"`

封面只承载这一条主信息；`overview` 仅用于底部"今日 N 条"的计数（`overview.length`），无需另写次要文案。
...
**核心理念**：封面在 App 列表页会被缩成小图，所以只承载**一条**信息——当天最大的爆点（`coverHook`），大到指甲盖尺寸也读得清。其余信息用**图形**表达，不用会糊成噪点的小字。
